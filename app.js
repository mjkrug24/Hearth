...
function restoreDeviceHome(){state.home=read('hearth-home',{tasks:[],groceries:[]});for(const kind of ['tasks','groceries'])state.home[kind]=(state.home[kind]||[]).map(item=>({...item,id:item.id||crypto.randomUUID(),kind}));}
async function loadHome(){try{const result=await api('/api/household');const wasShared=state.shared;state.shared=result.shared;if(result.shared){state.home={tasks:result.items.filter(x=>x.kind==='tasks'),groceries:result.items.filter(x=>x.kind==='groceries')};}else if(wasShared){restoreDeviceHome();}}catch(e){if(e.status===401||e.status===403){state.shared=false;restoreDeviceHome();}else if(state.shared)toast('Household refresh failed: '+e.message);}state.homeReady=true;renderHome();}
...
document.addEventListener('click',async e=>{
 ...
 if(b?.dataset.deleteItem||b?.dataset.editItem){const kind=b.dataset.kind,item=state.home[kind].find(x=>x.id===(b.dataset.deleteItem||b.dataset.editItem));if(b.dataset.deleteItem){try{await homeChange(kind,item,true);}catch{}}else{editingItem={...item,kind};$('#itemName').value=item.name;$('#itemDue').value=item.due||'';$('#itemQuantity').value=item.quantity||'';$('#itemDueLabel').classList.toggle('hidden',kind!=='tasks');$('#itemQuantityLabel').classList.toggle('hidden',kind!=='groceries');$('#itemError').textContent='';$('#itemDialog').showModal();}return;}
...
$('#disconnectBtn').onclick=async()=>{try{await post('/api/google/status',{},'DELETE');++syncSequence;state.connected=false;state.shared=false;restoreDeviceHome();state.events=[...state.localEvents];state.calendars=[localCalendar];$('#settingsDialog').close();await initConnection();toast('Disconnected in this browser');}catch(e){toast(e.message);}};
...
$('#itemForm').onsubmit=async e=>{e.preventDefault();const button=e.target.querySelector('.primary');button.disabled=true;try{await homeChange(editingItem.kind,{...editingItem,name:$('#itemName').value.trim(),due:$('#itemDue').value,quantity:$('#itemQuantity').value});$('#itemDialog').close();}catch(error){$('#itemError').textContent=error.message;}finally{button.disabled=false;}};
