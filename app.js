...
function displayEvents(){const from=dateKey(addDays(new Date(state.date.getFullYear(),state.date.getMonth(),1),-7)),to=dateKey(addDays(new Date(state.date.getFullYear(),state.date.getMonth()+1,1),190));return mergePending(expandLocal(state.events,from,to),outbox.forAccount(state.account));}
...
function deferDelete(op){const row=enqueue({...op,action:'delete',notBefore:Date.now()+10000});$('#undoBar span').textContent='Deleted '+op.item.name||op.item.title;$('#undoBar span').textContent='Removed “'+(op.item.title||op.item.name)+'” · undo before it syncs';$('#undoBar').classList.remove('hidden');$('#undoDelete').dataset.id=row.id;setTimeout(()=>{flushQueue();renderSyncDetails();},10100);}
