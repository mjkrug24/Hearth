...
test('Every recipe keeps portions that line up with its ingredient list',()=>{
 for(const recipe of recipes){
  assert(Number.isInteger(recipe.servings)&&recipe.servings>=1);
  assert.equal(recipe.portions.length,recipe.ingredients.length,recipe.name+' portion count');
  assert.deepEqual(recipe.portions.map(p=>p.name),recipe.ingredients,recipe.name+' portion names');
