...
export const storeTokens=(res,value)=>res.setHeader('Set-Cookie',`${cookieName}=${seal(value)}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=2592000`);
...
export const eventFromGoogle=e=>({id:`g_${e.id}`,googleEventId:e.id,title:e.summary||'(No title)',date:e.start.date||e.start.dateTime.slice(0,10),time:e.start.date?'':e.start.dateTime?.slice(11,16)||'',end:e.start.date?'':e.end.dateTime?.slice(11,16)||'',allDay:!!e.start.date,location:e.location||'',notes:e.description||'',calendar:'mike',updated:e.updated});
