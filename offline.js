...
export class Outbox{
 constructor(storage=localStorage){this.storage=storage;this.key='hearth-outbox-v1';this.running=false;this.revision=0;this.rows=this.read();}
 read(){try{return JSON.parse(this.storage.getItem(this.key))||[];}catch{return [];}}
 persist(){this.revision++;this.storage.setItem(this.key,JSON.stringify(this.rows));}
